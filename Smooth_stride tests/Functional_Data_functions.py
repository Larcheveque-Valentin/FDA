...
class TSCNN(nn.Module):
    def __init__(self, hyperparams,output_size):
        ...
        self.convlayer1=nn.Sequential(
            nn.Conv1d(self.n_channel*len(hyperparams.derivative),hyperparams.n_conv_in,kernel_size=hyperparams.kernel_size_1,stride=hyperparams.stride_1,padding=hyperparams.padding_1,dilation=hyperparams.dilation_1),
    ...
    def Granulator(self,x):
        ...
        if "inter" not in self.hyperparameters.Smoothing_type:     
            Recons_train=torch.zeros([x_grid.shape[0],self.n_channel*len(self.hyperparameters.derivative),self.hyperparameters.granulation]).float().cuda()
            i=0
            ...
            x.interpolation=self.hyperparameters.smoother.smoothing()
            eval_points=linspace(1,x.grid_points[0].shape[0],self.hyperparameters.granulation)
...
class FCNN(nn.Module):
    def __init__(self, hyperparams,output_size):
        ...
        self.fc_block=nn.Sequential(
            ...
            nn.Linear(hyperparams.n_Flat_out,hyperparams.output_size),
        )
    ...
    def train(n_epochs, module, optimizer, loss, batch_size):
        ...
        for epoch in (range(n_epochs)):
            ...
            if len(unique(y_train.cpu()))<y_train.shape[0]//2:
                ...
                mse_loss_test=nn.MSELoss()(module(x_test),y_test).unsqueeze(0).cpu()
    ...
    # Boucle pour créer chaque subplot
        ...
        for j,hyperparams in enumerate(params):
            ...
            for k,model in enumerate(models):
                ...
                spec_param[k].batch_size=X.shape[0]//3
                ...
                monte_carlo_test_acc[:,:,i,j,k],mean_acc_test[:,i,j,k],IC_acc_test[:,:,i,j,k]=Hyperparameter_Test(model_class=model,
